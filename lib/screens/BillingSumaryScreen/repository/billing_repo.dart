...
class BillingRepo {
  ...
  static Future<Result<List<CouponDataModel>>> getCoupons() async{
    try{
      ...
      Box customer_box = Hive.box("customer_box");
      String token = customer_box.get("token");
      var response = await http.post(Uri.parse('$apiJsURL//get-coupons'), body: params, headers: {'Authorization': 'Bearer $token'});
       List result = jsonDecode(response.body);
       for(int i = 0; i < result.length; i++){
        CouponDataModel coupon = CouponDataModel.fromJson(result[i]);
