...
class CalendarRepo {
  static Future<Result<List<CalendarDataModel>>> fetchCalendarDates(
      String cst_id, String subs_id) async {
    try {
      List<DateTime> dateList = [];
      List<CalendarDataModel> calendarData = [];
      Map<String, dynamic> params = {"cst_id": cst_id, "subs_id": subs_id};
      Box customer_box = Hive.box("customer_box");
      String token = customer_box.get("token");
      var response = await http.post(
          Uri.parse('$apiJsURL/get-order-by-Subscription'),
          body: params,
  ...
  static Future<Result<String>> cancelOrder(
    ...
    try {
      ...
      var response = await http.post(Uri.parse('$apiJsURL/cancel-order-date'),
